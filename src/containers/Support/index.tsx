...
const SupportPageButton: React.FC<SupportPageButtonProps> = ({ children, onClick }) => {
    return (
        <Button onClick={onClick} mt={4} width="100%" border={`1px solid ${theme.colors.brand[500]}`} background="white">
            <Text fontSize="12px">
                {children}
            </Text>
        </Button>
...
const KnowledgeBaseFAQs: React.FC<KnowledgeBaseFAQProps> = ({ FAQ, isWebView }) => {
    return (
        <Flex justifyContent="center" flexDirection="column" justify="space-between" border={`1px solid ${theme.colors.dimText}`} borderRadius={5} p={4} mb={4} background={theme.colors.accent[50]}>
            <Text fontWeight={600}>{FAQ?.Question}</Text>
            <Divider width={isWebView ? "30%" : "80%"} color={theme.colors.dimText} orientation="horizontal" />
            <Text color={theme.colors.dimText} fontSize={12}>{FAQ?.Answer}</Text>
        </Flex>
    );
};
...
const UserSupport: React.FC<UserSupportProps> = () => {
    ...
    return (
        <PageWrap title="Support Page" width="100%" alignSelf="center" script={zendeskWidgetScriptUrl} script_id="ze-snippet">
