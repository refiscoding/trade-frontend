...
const SelectPaymentComponent: React.FC<SelectPaymentComponentProps> = ({ mobileFlow, setShowPaymentOptions }) => {
    ...
    return (
        <Grid gridTemplateRows="40px 1fr 1fr" background={background} padding={padding} borderRadius={borderRadius} boxShadow={boxShadow}>
            {
                mobileFlow
                    ? (
                        <Flex>
                            <ArrowLeft />
                            <Text ml={3} fontWeight={600} onClick={setShowPaymentOptions}>Select Payment Method</Text>
                        </Flex>
                    )
                    : (
